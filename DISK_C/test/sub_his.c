...
void show_sub_history(int* page, USER* u,char*vn)
{
	...
	cleardevice();//库函数
	base_sub_draw_history(fp1, len,label,label2);
	base_sub_draw_my_history(u);
	...
	while (1)
	{
		...
		for (k = 0; k <j ; k++)
		{
			...
			if (MouseX > 24 * 4 * 4 + 25+24 && MouseX < 24 * 4 * 4 + 25+24+50 && MouseY>k * 27 && MouseY < k * 27 + 24)
			{
				
				if (mouse_press(24 * 4 * 4 + 25+24, k * 27, 24 * 4 * 4 + 25+24+50, k * 27+24) == 2)
				{
					MouseS = 1;
					MouseS = 1;
					MouseS = 1;
					...
					if (num == 0)
					{
						...
						base_sub_history_lightbutton_login_information(24 * 4 * 4 + 25+24, k * 27, 24 * 4 * 4 + 25+24+50, k * 27+24, WHITE, RED, 3, fp1,len,label,label2,q);//color1为线颜色，color2为填充色
				...
				else if (mouse_press(24 * 4 * 4 + 25+24, k * 27, 24 * 4 * 4 + 25+24+50, k * 27+24) == 1)
				{
					del(STK_ADR6,len,u,k,label2);
					MouseS = 0;
					*page=21;
					return;
				}
		...
		if (MouseX > 420+20+10 && MouseX < 530+20+10 && MouseY>300 && MouseY < 350)//返回首页判定
		{
			if (mouse_press(420+20+10, 300, 530+20+10, 350) == 2)
			{
				...
				{
					MouseS = 1;
					base_sub_history_lightbutton_login_information(420+20+10, 300, 530+20+10, 350, WHITE, RED, 4,fp2,len,label,label2,q);//color1为线颜色，color2为填充色
					num = 4;
				}
				continue;
			}
		...
		if (MouseX > 420+20+10-250-25 && MouseX < 530+20+10-250-25 && MouseY>300 && MouseY < 350)//上一页判定
		{
			if (mouse_press(420+20+10-250-25, 300, 530+20+10-250-25, 350) == 2)
			{
				if (num == 0)
				{
					...
					{
						MouseS = 1;
						base_sub_history_lightbutton_login_information(420+20+10-250-25, 300, 530+20+10-250-25, 350, WHITE, RED, 5,fp2,len,label,label2,q);//color1为线颜色，color2为填充色
						num = 5;
					}
		...
		if (MouseX > 420+20+10-125 && MouseX < 530+20+10-125 && MouseY>300 && MouseY < 350)//下一页判定
		{
			if (mouse_press(420+20+10-125, 300, 530+20+10-125, 350) == 2)
			{
				if (num == 0)
				{
					...
					{
						MouseS = 1;
						base_sub_history_lightbutton_login_information(420+20+10-125, 300, 530+20+10-125, 350, WHITE, RED, 6,fp2,len,label,label2,q);//color1为线颜色，color2为填充色
						num = 6;
					}	
				}
				continue;
		...
		/*if (MouseX > 25 && MouseX < 110+25 && MouseY>300 && MouseY < 350)//编辑判定
		{
			if (mouse_press(25, 300, 110+25, 350) == 2)
			{
				...
				{
					MouseS = 1;
					base_sub_history_lightbutton_login_information(25, 300, 110+25, 350, WHITE, RED, 7,fp1,len,label,label2,q);//color1为线颜色，color2为填充色
					num = 7;
				}
				continue;
			}
		...
		if (MouseX > 24 * 4 * 4 + 25+24 && MouseX < 24 * 4 * 4 + 25+24+50 && MouseY>ab * 27 && MouseY < ab * 27 + 24)
		{
			if (mouse_press(24 * 4 * 4 + 25+24, ab * 27, 24 * 4 * 4 + 25+24+50, ab * 27+24) == 2)
			{
				continue;
			}
		}
		...
		if (num != 0)//点亮恢复
		{
			...
			{
				//fp1 = fopen(STK_ADR6, "rb+");
				base_sub_history_recoverbutton_login_information(num, fp1, ab,len,label,label2,q);//恢复
			}
			num = 0;
		}
...
void base_sub_draw_history(FILE* fp1, int len,int label,int label2)
{
	...
	for (i=0; i < 10; i++)
	{
		...
		bar(24 * 4 * 4 + 25+24, i * 27, 24 * 4 * 4 + 25+24+50, i * 27+24);//
		puthz(24 * 4 * 4 + 25+24, i*27, "删除", 24, 24 + 2, WHITE);


		setcolor(RED);//blibli图案
		rectangle(24 * 3, i * 27, 24 * 4 * 4 + 25+26, i * 27 + 24);
		rectangle(24 * 4 * 4 + 25+24, i * 27, 24 * 4 * 4 + 25+24+50, i * 27+24);
...
void base_sub_history_lightbutton_login_information(int x, int y, int x1, int y1, int color1, int color2, int flag, FILE* fp1,int len,int label,int label2,int q)//按钮点亮
{
	int i;
	HIS* bh;
...
void base_sub_history_recoverbutton_login_information(int status, FILE* fp1,int i,int len,int label,int label2,int q)//恢复
{
	HIS* bh;
...
void del(char*STK_ADR6,int len,USER*u,int del_num,int label2)
{
	...
	for (i=0;i<len;i++)
	{
		if(i==len-del_num-1-(label2-1)*10)
		{
			continue;
		}
		fseek(fp1,i * sizeof(HIS),SEEK_SET);
		...
		fread(bh,sizeof(HIS),1,fp1);
		fwrite(bh,sizeof(HIS),1,fp2);
	...
	for (i=0;i<len-1;i++)
	{
		...
		fseek(fp2,i*sizeof(HIS),SEEK_SET);
		fseek(fp1,0,SEEK_END);
		if (( bh= (HIS*)malloc(sizeof(HIS))) == NULL)
		{
			printf("memoryallocation runs wrong in lgfunc.c");
		...
		fread(bh,sizeof(HIS),1,fp2);
		fwrite(bh,sizeof(HIS),1,fp1);
