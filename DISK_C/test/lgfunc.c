...
void input_database(char* name, char* id, char* code, char* bank)
{
	...
	strcpy(STK_ADR4, STK_ADR);
	strcpy(STK_ADR5, STK_ADR);
	strcpy(STK_ADR6, STK_ADR); //遇到'\0'结束
								//遇到'\0'结束
								//遇到'\0'结束
	strcat(STK_ADR, "history.dat");
	strcat(STK_ADR2, "collect.dat");
	strcat(STK_ADR3, "ntlook.dat");
	strcat(STK_ADR4, "plun.dat");
	strcat(STK_ADR5, "dzan.dat");
	strcat(STK_ADR6, "dmu.dat");
	...
	if ((fp5 = fopen(STK_ADR5, "wb+")) == NULL)
	{
		printf("%s", "create fail");
	}
	if ((fp6 = fopen(STK_ADR6, "wb+")) == NULL)
	{
		printf("%s", "create fail");
	}
